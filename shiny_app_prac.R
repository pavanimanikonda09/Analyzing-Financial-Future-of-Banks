...
ui <- fluidPage(
  navbarPage("Churn!",
             ...
             navbarMenu("Models",
                        ...
                        tabPanel("XAI for Random Forest",
                                 mainPanel("model_Summary",
                                           verbatimTextOutput("ran_for_explainable")
                                 )
...
server <- function(input, output) {
  ...
  output$ran_for_explainable <- renderPrint({
    inFile <- input$file1
    if (is.null(inFile))
      return(NULL)
    data = read.csv(inFile$datapath, header = input$header)
    ...
    explainer <- DALEX::explain(model = rf, data = testing_data[, -ncol(testing_data)], y = answer, type = "classification")
    
    # Generate feature importance
    feature_importance <- model_parts(explainer)
    
    # Generate individual predictions explanations (e.g., SHAP values) for a specific observation
    new_observation <- testing_data[1, -ncol(testing_data)]  # Change this to the observation you want to explain
    predictions_explanations <- predict_parts(explainer, new_observation = new_observation)
    # Create a model-level summary
    model_summary <- model_profile(explainer)
    model_summary
